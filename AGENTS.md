...
TL;DR: Keep command output short. Do not take actions unless asked. Do not waste
tokens on experiments. Do only what is asked.

# COMMUNICATION STYLE

- Use English by default; if the user writes in another language, use that
  language
...
- ALWAYS tell apart an observation from an action request: observation ("works
  oddly") → discuss, DO NOT fix request ("fix this") → act
